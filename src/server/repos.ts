...
import { parseFloorplanDraft, readStoredFloorplan, writeStoredFloorplan } from "../data/floorplan.ts";
...
export const floorplan = {
  ...
  async create(ctx: RequestContext, eventId: string, draft: FloorplanDraft): Promise<Floorplan> {
    ...
    const parsed = parseFloorplanDraft(draft);
    const id = newId("fp");
    const updatedAt = new Date();
    const stored = writeStoredFloorplan(parsed);
    const [saved] = await db.batch([
      db
        .insert(s.floorplans)
        .values({ id, eventId, workspaceId: ctx.workspaceId, name: parsed.name, items: stored, updatedAt })
        .returning(),
      db.insert(s.eventHistory).values(
        historyValues(ctx, {
          eventId,
          ...
          action: "created",
          before: null,
          after: { id, eventId, ...parsed, updatedAt: updatedAt.toISOString(), ...context },
        }),
      ),
    ]);
  ...
  async save(ctx: RequestContext, id: string, draft: FloorplanDraft): Promise<Floorplan> {
    ...
    const [saved] = await db.batch([
      db
        .update(s.floorplans)
        .set({ name: parsed.name, items: stored, updatedAt })
        .where(and(eq(s.floorplans.id, id), eq(s.floorplans.workspaceId, ctx.workspaceId)))
        .returning(),
      db.insert(s.eventHistory).values(
        historyValues(ctx, {
          ...
          after: { id, eventId: before.eventId, ...parsed, updatedAt: updatedAt.toISOString(), ...context },
        }),
      ),
    ]);
    return map.toFloorplan(saved[0]!);
...
export const teamUpdates = {
  ...
  async create(ctx: RequestContext, eventId: string, body: Body): Promise<TeamUpdate> {
    ...
    if (body.notifyTeam === false) return update;
