...
export function toRfp(row: InferSelectModel<typeof s.rfps>): Rfp {
  return {
    ...
    roomBlockRequired: row.roomBlockRequired,
    roomsRequired: row.roomsRequired,
    checkInDate: iso(row.checkInDate),
    checkOutDate: iso(row.checkOutDate),
    spaceRequirements: row.spaceRequirements,
    foodBeverageSpendCents: row.foodBeverageSpendCents,
    ancillarySpendCents: row.ancillarySpendCents,
    ancillarySpendNotes: row.ancillarySpendNotes,
    city: row.city,
